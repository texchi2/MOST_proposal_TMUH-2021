...
\begin{outline}
HNSCC SVS (Aperio slides) is available at https://github.com/texchi2/HE2RNA\_code/blob/master/gdc\_manifests\\
/gdc\_manifest.2018-06-26\_diagnostic\_TCGA-HNSC.txt
...
\1 Aim 3: Biomarkers Discovery from RNA-Seq, Whole-slide Images, and EHR from TMU cohort\\
...
$Y_{mrna} = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + ... + \beta_n X_n + \epsilon$\\[0.5cm]
While $X_1, X_2, ...X_n$ are the features extracted from whole slide images of pathology.
...
Apply model on validation cohort of TMU HNSCC (with survival outcome):
%Y (survival) ← X (slide represents gene expression)
$Y_{survival} = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + ... + \beta_n X_n + \epsilon$\\[0.5cm]
While $X_1, X_2, ...X_n$ are the features extracted from whole slide images of pathology.
...
\smartdiagramadd[flow diagram:horizontal]{
 TMU cohort (WSI/EHR), HE2RNA, RNA-Seq (Representation)
}{
...
\bcaption{Workflow of RNA-Seq representation of TMU cohort}
%\acrshort{hnscc} Cox's hazard ratio and \textit{P} value plot.
{The deep learning model of HE2RNA, which was trained under TCGA/TCIA \acrshort{hnscc} cohort, could be applied for RNA-Seq prediction for TMU cohort. The representation will be used for subsequent survival analysis.\\
(
TCGA: \acrlong{tcga};
TCIA: \acrlong{tcia};

...
%\subsection{Surgical Margin Issue}
%在許多已發表的 TCGA 分析的研究中，有些文章沒有辨明它，有的文章則認為對預後不重要(not significant)，
%to prove my hypothesis:
%但接下來我要用力證明：這將會造成 biomarker discovery 的 bias, 因為 survival or prognosis 會有非常大差別，所以在研究中要 only R0 案例」。
%=> not really
%it should include all R* status, and see margins positive as an independent biomarker for the prognosis of \acrshort{hnscc} patients \citep{SLAUGHTER1953} \citep{Ha2003} \citep{Bilde2009} \citep{Tabor2001} \citep{Reis2011} \citep{Chi2020}.
Thus, we plan to investigate how deep learning of pathology could "see" TP53 gene mutation or other molecular abnormalities around the surgical margin of specimen.
%\paragraph{Prognosis}
...
image-based phenotypes could be correlated with their underlying genomics data 
Deep learning algorithm could potentially extract molecular phenotypes from pathology images.
A HE2RNA model, developed by Schmauch and their colleague\citep{Schmauch2020},  predicts RNA-Seq profiles from whole-slide images of TCIA. It has been validated by CD3- and CD20-\acrshort{ihc} staining. The transcriptomic representation learned by HE2RNA can also be transferred on other datasets.
...
the ability of HE2RNA to focus specifically on cancer-related molecular information\citep{Schmauch2020}. A high expression level of MKI67 is correlated with the most advanced stages of liver cancer.

%why deep learning?
% NCI's recommendation:
Aiding the Genomics Characterization of Tumors: 
...
\begin{outline}
HNSCC SVS (Aperio slides)
https://github.com/texchi2/HE2RNA_code/blob/master/gdc_manifests/gdc_manifest.2018-06-26_diagnostic_TCGA-HNSC.txt
...
\1 Aim 1: Construct a Deep Learning Platform of TCIA Survival Analysis\\
...
We further plan to develop a deep-learning based data mining as well as validation platform. It might incorporate whole slide images of pathology from the Cancer Imaging Archive (TCIA) and 
an independent dataset of ??? slides from 46 patients with HNSCC from Joint Biobank of Taipei Medical University (TMU dataset).
...
RNA-Seq:  we already save a file containing transcriptomes matched to whole-slide images, available at
https://github.com/texchi2/HE2RNA_code/blob/master/transcriptome_data.py
...
\1 Aim 2: Construct a Deep Learning Platform of EHR Survival Analysis\\
...
poster https://wiki.cancerimagingarchive.net/download/attachments/1966514/TCGA_TCIA_Poster_111511_HHFa_WIDE.pptx?version=1&modificationDate=1323207330686&api=v2

*** IL19 is one of our suggested candidate marker.
TCPA modeling: Y (RNA-seq) ← X (slide represents gene expression)
Apply model on TMUH HNSCC test cohort (with survival outcome):
Y (survival) ← X (slide represents gene expression)
(as IL19 validation, to see is it still a biomarker)
Please see Figure \ref{fig:HE2RNA}.
...
\1 Aim 4: Biomarkers Discovery from RNA-Seq, Whole-slide Images, and EHR from TCGA and TMU biobank\\
